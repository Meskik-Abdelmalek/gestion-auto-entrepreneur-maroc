...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    verifyCsrf();
    $num=$_POST['invoice_number']??nextInvoiceNumber(); $date=clean($_POST['invoice_date']??date('Y-m-d'));
    $due=clean($_POST['due_date']??''); $clientN=clean($_POST['client_name']??'');
    $clientId=(int)($_POST['client_id']??0)?:null; $cat=clean($_POST['category']??'Service');
    $status=clean($_POST['status']??'En attente'); $payDate=clean($_POST['payment_date']??'');
    $payMode=clean($_POST['payment_method']??''); $notes=clean($_POST['notes']??'');
    $linesIn=$_POST['lines']??[];
    $total_ht=0;
    foreach ($linesIn as $l) $total_ht+=(float)($l['qty']??0)*(float)($l['price']??0);
    if ($clientN && $total_ht>0) {
        $quarter=getQuarter($date); $fy=(int)date('Y',strtotime($date));
        $db->prepare("INSERT INTO ae_invoices (invoice_number,client_id,client_name,invoice_date,due_date,category,amount_ht,has_tva,amount_ttc,status,payment_date,payment_method,quarter,fiscal_year,notes) VALUES (?,?,?,?,?,?,?,0,?,?,?,?,?,?,?)")
           ->execute([clean($num),$clientId,$clientN,$date,$due?:null,$cat,$total_ht,$total_ht,$status,$payDate?:null,$payMode?:null,$quarter,$fy,$notes]);
        $invId=(int)$db->lastInsertId();
        foreach ($linesIn as $i=>$l) {
            $desc=clean($l['desc']??''); $qty=(float)($l['qty']??0); $price=(float)($l['price']??0);
            if ($desc||$qty>0) $db->prepare("INSERT INTO ae_invoice_lines (invoice_id,description,quantity,unit_price,amount,sort_order) VALUES (?,?,?,?,?,?)")->execute([$invId,$desc,$qty,$price,$qty*$price,$i]);
        }
        if ($status==='En attente') $db->prepare("INSERT INTO ae_reminders (invoice_id) VALUES (?)")->execute([$invId]);
        $chk=$db->prepare("SELECT id FROM ae_clients WHERE name=?"); $chk->execute([$clientN]);
        if (!$chk->fetchColumn()) $db->prepare("INSERT INTO ae_clients (name) VALUES (?)")->execute([$clientN]);
        flash('message',"Facture ".clean($num)." créée !"); header("Location: /invoice-view.php?id=$invId"); exit;
    }
    $error="Renseignez le client et au moins une ligne de prestation.";
}
...
<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-f p-5">
    <h2 class="font-semibold text-sm text-fluent-neutral mb-4 flex items-center gap-2"><span class="w-6 h-6 bg-fluent-blue rounded-lg flex items-center justify-center text-white text-xs font-bold">1</span>Informations Générales</h2>
    <div class="grid grid-cols-2 gap-3">
        <div><label class="block text-xs font-medium text-fluent-n2 mb-1">N° Facture *</label>
            <input name="invoice_number" value="<?= h($nextNum) ?>" required class="w-full px-3 py-2.5 text-sm border border-fluent-n4 dark:border-white/20 rounded-xl inp bg-white dark:bg-gray-700 font-mono"></div>
        <div><label class="block text-xs font-medium text-fluent-n2 mb-1">Date *</label>
            <input type="date" name="invoice_date" value="<?= date('Y-m-d') ?>" required onchange="updateDue(this.value)" class="w-full px-3 py-2.5 text-sm border border-fluent-n4 dark:border-white/20 rounded-xl inp bg-white dark:bg-gray-700"></div>
        <div><label class="block text-xs font-medium text-fluent-n2 mb-1">Échéance</label>
            <input type="date" name="due_date" id="inv-due" value="<?= date('Y-m-d',strtotime('+30 days')) ?>" class="w-full px-3 py-2.5 text-sm border border-fluent-n4 dark:border-white/20 rounded-xl inp bg-white dark:bg-gray-700"></div>
        <div><label class="block text-xs font-medium text-fluent-n2 mb-1">Catégorie</label>
            <select name="category" onchange="calcTotal()" class="w-full px-3 py-2.5 text-sm border border-fluent-n4 dark:border-white/20 rounded-xl inp bg-white dark:bg-gray-700">
                <option value="Service">Service (IR 1%)</option><option value="Commerce">Commerce (IR 0.5%)</option><option value="Industrie">Industrie</option></select></div>
    </div>
</div>

<!-- Step 2: Client -->
<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-f p-5">
    <h2 class="font-semibold text-sm text-fluent-neutral mb-4 flex items-center gap-2"><span class="w-6 h-6 bg-fluent-blue rounded-lg flex items-center justify-center text-white text-xs font-bold">2</span>Client</h2>
    <div class="relative">
        <input name="client_name" id="client-inp" value="<?= h($preClient) ?>" autocomplete="off" required placeholder="Nom ou raison sociale…"
    ...
    <p class="text-xs text-fluent-n3 mt-1.5">Les nouveaux clients sont ajoutés automatiquement à votre annuaire.</p>
</div>
...
<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-f p-5">
    ...
    <div id="lines-wrap" class="space-y-2">
        <div class="line-row grid grid-cols-12 gap-2 items-center">
            <div class="col-span-12 sm:col-span-6"><input name="lines[0][desc]" placeholder="Ex: Développement site web…" class="w-full px-3 py-2.5 text-sm border border-fluent-n4 dark:border-white/20 rounded-xl inp bg-white dark:bg-gray-700 placeholder-fluent-n3"></div>
    ...
    <div class="flex flex-col items-end gap-1 mt-4 pt-4 border-t border-fluent-n5 dark:border-white/10">
        <div class="flex justify-between w-56 text-sm"><span class="text-fluent-n3">Sous-total HT</span><span class="font-semibold text-fluent-neutral" id="ttl-ht">0,00 MAD</span></div>
        ...
        <div class="text-xs text-amber-500 font-medium mt-0.5" id="ir-hint"></div>
    </div>
</div>

<!-- Step 4: Payment -->
<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-f p-5">
    <h2 class="font-semibold text-sm text-fluent-neutral mb-4 flex items-center gap-2"><span class="w-6 h-6 bg-fluent-blue rounded-lg flex items-center justify-center text-white text-xs font-bold">4</span>Paiement & Statut</h2>
    <div class="grid grid-cols-2 gap-3">
        <div><label class="block text-xs font-medium text-fluent-n2 mb-1">Statut</label>
            <select name="status" onchange="document.getElementById('pay-row').classList.toggle('hidden',this.value!=='Payé')" class="w-full px-3 py-2.5 text-sm border border-fluent-n4 dark:border-white/20 rounded-xl inp bg-white dark:bg-gray-700">
                <option>En attente</option><option>Payé</option><option>Annulé</option></select></div>
        <div><label class="block text-xs font-medium text-fluent-n2 mb-1">Mode</label>
            <select name="payment_method" class="w-full px-3 py-2.5 text-sm border border-fluent-n4 dark:border-white/20 rounded-xl inp bg-white dark:bg-gray-700">
                <option value="">—</option><?php foreach (['Virement','Chèque','Espèces','CB','PayPal','Mobile Money'] as $m): ?><option><?= $m ?></option><?php endforeach; ?></select></div>
        <div id="pay-row" class="col-span-2 hidden"><label class="block text-xs font-medium text-fluent-n2 mb-1">Date Paiement</label>
            <input type="date" name="payment_date" value="<?= date('Y-m-d') ?>" class="w-full px-3 py-2.5 text-sm border border-fluent-n4 dark:border-white/20 rounded-xl inp bg-white dark:bg-gray-700"></div>
        <div class="col-span-2"><label class="block text-xs font-medium text-fluent-n2 mb-1">Notes</label>
            <textarea name="notes" rows="2" placeholder="Référence commande, conditions…" class="w-full px-3 py-2.5 text-sm border border-fluent-n4 dark:border-white/20 rounded-xl inp bg-white dark:bg-gray-700 resize-none placeholder-fluent-n3"></textarea></div>
    </div>
</div>

<div class="flex gap-3 justify-end pb-4">
    <a href="/invoices.php" class="btn-f px-5 py-2.5 text-sm border border-fluent-n4 dark:border-white/20 rounded-xl text-fluent-n2 dark:text-gray-300 hover:bg-fluent-n6 dark:hover:bg-white/10 bg-white dark:bg-gray-800">Annuler</a>
    <button type="submit" class="btn-f px-6 py-2.5 text-sm bg-fluent-blue text-white rounded-xl font-semibold shadow-f hover:bg-fluent-blue-dk">Créer la Facture →</button>
</div>
</form>
</div>
<script>
let li=1;
const IR={Service:<?= $cfg['ir_rate_services']??0.01 ?>,Commerce:<?= $cfg['ir_rate_commerce']??0.005 ?>,Industrie:0};
function getIR(){return IR[document.querySelector('[name="category"]')?.value]||0;}
...
function calcTotal(){
    ...
    const ir=t*getIR(); document.getElementById('ir-hint').textContent=t>0?`IR estimé: ${fmt(ir)} (${(getIR()*100).toFixed(1)}%)`:'';
}
function addLine(){
...
function updateDue(v){if(!v)return;const d=new Date(v);d.setDate(d.getDate()+30);document.getElementById('inv-due').value=d.toISOString().split('T')[0];}
